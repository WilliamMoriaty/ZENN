#!/usr/bin/env python3
"""
SmolLM2-360M training on AG News with DDP and performance optimizations
...
def train_worker(rank, world_size, args):
    ...
    if is_main_process():
        print(f"\n{'='*80}")
        print(f"SmolLM2-360M Training on AG News with DDP")
        print(f"{'='*80}")
    ...
    if is_main_process():
        print(f"  ✓ Loaded tokenizer from HuggingFaceTB/SmolLM2-360M")
    ...
    if is_main_process():
        print(f"  ✓ Loaded SmolLM2-360M from HuggingFaceTB/SmolLM2-360M (safetensors)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train SmolLM2-360M on AG News with DDP')
