#!/usr/bin/env python3
"""
SmolLM2-360M training on BBC News with DDP and LoRA support
...
# ============================================================================
# Using SmolLM2-360M from Transformers
# ============================================================================
# We use AutoModelForSequenceClassification with SmolLM2-360M
...
def plot_training_curves(train_losses, test_accuracies, learning_rates, epoch_times, save_path):
    ...
    info_text += f"Dataset: BBC News\n"
    info_text += f"Model: SmolLM2-360M\n\n"
    info_text += f"Results:\n"
...
def train_worker(rank, world_size, args):
    ...
    if is_main_process():
        print(f"\n{'='*80}")
        print(f"SmolLM2-360M Training on BBC News with DDP")
        print(f"{'='*80}")
    ...
    if is_main_process():
        print(f"\nLoading SmolLM2-360M tokenizer from Hugging Face Hub...")
    
    # Use SmolLM2-360M model
    ...
    # Load datasets
    if is_main_process():
        print(f"Loading BBC News dataset...")
    
    train_dataset = BBCNewsDataset('train', tokenizer, max_length=args.max_length, token=hf_token)
    test_dataset = BBCNewsDataset('test', tokenizer, max_length=args.max_length, token=hf_token)
    ...
    # Create SmolLM2-360M model from Hugging Face Hub
    if is_main_process():
        print(f"\nInitializing SmolLM2-360M model from Hugging Face Hub...")
    
    # Load model
    model = AutoModelForSequenceClassification.from_pretrained(
        model_name,
        num_labels=5,  # BBC News has 5 classes
        token=hf_token,
    ...
    if is_main_process():
        print(f"  ✓ Loaded SmolLM2-360M model from {model_name}")
    ...
    if args.use_lora:
        ...
        if not PEFT_AVAILABLE:
            if is_main_process():
                print(f"\n{'='*70}")
                print(f"ERROR: LoRA requires PEFT package!")
                print(f"{'='*70}")
                print(f"Please install it using:")
                print(f"  pip install peft")
                print(f"{'='*70}\n")
            raise ImportError("PEFT is required for LoRA but not installed")
        
        if is_main_process():
            print(f"\nApplying LoRA configuration...")
        
        # SmolLM2 is based on Llama architecture
        # Common Llama attention modules: q_proj, k_proj, v_proj, o_proj
        if args.lora_target_modules:
            target_modules = args.lora_target_modules.split(',')
        else:
            # Default Llama/SmolLM2 attention modules
    ...
    # Note: LoRA + gradient checkpointing + DDP can cause issues
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train SmolLM2-360M on BBC News with DDP and LoRA support')
    ...
    parser.add_argument('--lora-target-modules', type=str, default='', 
                        help='Comma-separated list of target modules for LoRA (default: q_proj,v_proj for SmolLM2/Llama)')
